...
public class script {
     ...
     *
     * 记忆化：
     *    省去哪些不必要的计算，就好比计算 398 * 434 你需要一张纸盒一支笔或则一个计算器，
     *    但是计算 4 * 8 就直接能知道 32， 那是因为你记得乘法口诀表，这就是记忆化的一个好处
     *
     * 多参中的记忆化
     *    这本书中写到 『世界上并没有函数接受多个参数』函数式一个集合到另一个集合的应用，不能有多个参数
     *    多个参数的函数式以下情况之一：
     *         1 元组函数
     *         2. 函数的函数的函数，也就是柯里化
     *
     * 记忆即是关于保持函数调用之间的状态，记忆函数则是行为取决于当前状态的函数，但是它总是会返回相同的参数返回相同的
     **/

    /*
            1. 递归函数式一个定义为应用自己的函数
            2. 再Java 中，递归方法再递归调用自己之前将当前计算状态推入栈
            3. Java 默认的栈的大小事有限的，它可以配置的很大，但通常只是浪费空间，因为所有的线程都是使用相同的栈大小
            4. 尾递归函数式递归调用发生在最后位置的函数
            5. 再某些语言中，递归函数使用尾调用消除来优化
            6. Java 没有 尾调用消除来优化
            7. lambda 可以递归
            8. 记忆可以加速函数的计算
     */
